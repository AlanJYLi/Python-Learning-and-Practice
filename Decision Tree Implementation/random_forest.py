...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        for a in range(n):
            i = np.random.randint(0, n)  # generate a random row number to draw the data point
            column_num = len(XX[0])
            samples.append(XX[i][0:column_num-1])
            labels.append(XX[i][column_num-1])
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                for tree in self.decision_trees:
                	store.append(tree.classify(record))  # use the prediction from the random forests
                y = np.append(y, np.argmax(np.bincount(store)))
            else:
